 ...
 * @return {boolean}
 */
function comparePreserveAlphaChannel(nodeColor, targetColor, preserveAlphaChannel) {
  if (preserveAlphaChannel) {
    return nodeColor
      .filter((item, index, array) => array[array.length - 1] !== item)
      .every((c, i) => parseFloat(c.value) === targetColor[i]);
  }
}
...
function applyColorFunction(node, initialColor, finalColor, preserveAlphaChannel) {
  ...
  if (comparePreserveAlphaChannel(nodeColor, initialColor[name], preserveAlphaChannel)) {
