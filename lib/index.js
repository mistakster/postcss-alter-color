...
function applyColorFunction(node, initialColor, finalColor, alphaChannel) {
  ...
  if (!alphaChannel && compareColors(nodeColor, initialColor[name])) {
    nodeColor.forEach((c, i) => c.value = finalColor[name][i]);
  } else if(alphaChannel && compareColors(sliceNodeColor, sliceInitialColor)) {
    ...
    root.walkRules(rule => {
      rule.walkDecls(decl => {
        ...
        cssTree.walk(parsedValue, {
          visit: 'Function',
          enter(node) {
            applyColorFunction(node, initialColor, finalColor, options.alphaChannel);
          }
        });
